...
# the shared ci-cpu.sif bakes scitex-dev[all,dev] DEPS, NOT scitex-math's —
...
# so only scitex-math's own thin dep set is fetched per run.
#
...
export TMPDIR="/tmp/ci-scitex_math-${GITHUB_RUN_ID:-0}-${GITHUB_RUN_ATTEMPT:-0}-$V"
rm -rf "$TMPDIR"
...
# scitex-math's reproducibility tests (validate_recipe renders the SAME recipe
...
# Install scitex-math + its [all,dev] extras WITH deps into the writable target.
# Fallback chain mirrors scitex-math's historical bare-uv/pip workflow so a
...
# scitex-math's suite is ~2460 tests; single-process it overran the job's old
...
# scitex-math style-stack are naturally isolated per worker — the safe way to
# parallelise a matplotlib-heavy suite.
#
# Worker count: use ALL cores. Each matrix leg now runs on its own dedicated
# self-hosted node (one runner per node: scitex-math-01/02/03), so there is no
...
# loadscope pins an entire MODULE's tests to ONE worker — and scitex-math's heavy
...
exec nice -n 19 ionice -c 3 \
    ...
    --cov=src/scitex_math --cov-report=xml --cov-report=term \
    -p no:cacheprovider
